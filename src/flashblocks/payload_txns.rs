use {
	rblib::{
		...
		reth::{
			optimism::txpool::OpPooledTx,
			payload::util::PayloadTransactions,
...
/// A wrapper around [`PayloadTransactions`] that drains previously observed
/// transactions before yielding new transactions.
	...
	>,
{
	...
	pub fn take_observed(
		self,
	) -> (impl Iterator<Item = TxHash>, impl Iterator<Item = TxHash>) {
